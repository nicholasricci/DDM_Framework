...
#define WORKING_DIRECTORY "TestsInstances"
...
int main (const int argc, const char * argv[]){
	...
	//Create folder that contain all test instances
	mkdir(WORKING_DIRECTORY, 0777);
	//strcpy(WorkingDirectory, WORKING_DIRECTORY);
	
	//Create istance folder
	//strcpy(WorkingDirectory, argv[instanceName]);
	
	sprintf(WorkingDirectory, "%s/%s", WORKING_DIRECTORY, argv[instanceName]);
