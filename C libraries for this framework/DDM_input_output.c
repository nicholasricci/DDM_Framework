...
DDM_Input* DDM_Initialize_Input(int argc, char* argv[]){
    ...
    if ((init_bit_matrix(&(ddm_input->result), ddm_input->updates, ddm_input->subscriptions, uninitialized)) != err_none){
...
void DDM_AND_Op_With_Bitmatrix(DDM_Input *ddm_input, const bitmatrix mask){
    ...
    for (i = 0; i < (uint64_t)ddm_input->updates; ++i)
        vector_bitwise_and(ddm_input->result[i], mask[i], ddm_input->subscriptions);
}
